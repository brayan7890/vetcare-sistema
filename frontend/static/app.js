...
async function api(path,opts={}){const h={"Content-Type":"application/json",...opts.headers};if(token)h["Authorization"]="Bearer "+token;const r=await fetch(API+path,{...opts,headers:h});if(r.status===401){logout();throw new Error("Sesion expirada")}if(r.status===204)return null;const txt=await r.text();let d;try{d=JSON.parse(txt)}catch{throw new Error(txt||"Error del servidor")}if(!r.ok){let m="Error";if(d.detail){m=typeof d.detail==="string"?d.detail:Array.isArray(d.detail)?d.detail.map(x=>x.msg||x).join("; "):JSON.stringify(d.detail)}throw new Error(m)}return d}
...
function filterTable(tid,q){const rows=document.getElementById(tid).querySelectorAll("tr");const lv=q.toLowerCase();rows.forEach(r=>{if(r.closest("thead"))return;r.style.display=r.textContent.toLowerCase().includes(lv)?"":"none"})}
const _mh={};function regMH(t,fn){_mh[t]=fn}
...
function closeModal(t){document.getElementById("modal-"+t).classList.add("hidden");const frm=document.getElementById("form-"+t);if(frm){frm.reset();if(_mh[t])frm.onsubmit=_mh[t]}if(t==="comprobante"){document.getElementById("detalles-container").innerHTML="";updateTotals();_cobrarHistorialId=null;var ce=document.getElementById("cb-cliente");var cr=document.getElementById("cb-ruc");if(ce)ce.readOnly=false;if(cr)cr.readOnly=false}}
...
async function openEditCita(id){try{const ci=await api("/citas/"+id);await fillPacienteSelect("c-paciente");const dt=new Date(ci.fecha);const local=dt.getFullYear()+"-"+String(dt.getMonth()+1).padStart(2,"0")+"-"+String(dt.getDate()).padStart(2,"0")+"T"+String(dt.getHours()).padStart(2,"0")+":"+String(dt.getMinutes()).padStart(2,"0");document.getElementById("c-paciente").value=ci.paciente_id;document.getElementById("c-fecha").value=local;document.getElementById("c-motivo").value=ci.motivo;regMH("cita",submitCita);const frm=document.getElementById("form-cita");frm.onsubmit=async function(ev){ev.preventDefault();try{await api("/citas/"+id,{method:"PUT",body:JSON.stringify({fecha:new Date(document.getElementById("c-fecha").value).toISOString(),motivo:document.getElementById("c-motivo").value,paciente_id:parseInt(document.getElementById("c-paciente").value)})});showToast("Cita actualizada");closeModal("cita");loadCitas()}catch(err){showToast(err.message,false)}};openModal("cita")}catch(e){showToast(e.message,false)}}
...
async function openEditHistorial(id){try{const h=await api("/historial/"+id);await fillPacienteSelect("h-paciente");document.getElementById("h-paciente").value=h.paciente_id;document.getElementById("h-motivo").value=h.motivo_consulta;document.getElementById("h-diagnostico").value=h.diagnostico||"";document.getElementById("h-tratamiento").value=h.tratamiento||"";document.getElementById("h-temp").value=h.temperatura||"";document.getElementById("h-fc").value=h.frecuencia_cardiaca||"";document.getElementById("h-peso").value=h.peso_kg||"";document.getElementById("h-obs").value=h.observaciones||"";document.getElementById("h-proxima").value=h.proxima_cita?new Date(h.proxima_cita).toISOString().slice(0,16):"";regMH("historial",submitHistorial);const frm=document.getElementById("form-historial");frm.onsubmit=async function(ev){ev.preventDefault();const pid=parseInt(document.getElementById("h-paciente").value);try{await api("/historial/"+id,{method:"PUT",body:JSON.stringify({motivo_consulta:document.getElementById("h-motivo").value,diagnostico:document.getElementById("h-diagnostico").value,tratamiento:document.getElementById("h-tratamiento").value||null,temperatura:document.getElementById("h-temp").value||null,frecuencia_cardiaca:document.getElementById("h-fc").value||null,peso_kg:parseFloat(document.getElementById("h-peso").value)||null,observaciones:document.getElementById("h-obs").value||null,proxima_cita:document.getElementById("h-proxima").value?new Date(document.getElementById("h-proxima").value).toISOString():null})});showToast("Registro actualizado");closeModal("historial");loadHistorial()}catch(err){showToast(err.message,false)}};openModal("historial")}catch(e){showToast(e.message,false)}}
...
async function openEditInventario(id){try{const x=await api("/inventario/"+id);document.getElementById("i-nombre").value=x.nombre;document.getElementById("i-categoria").value=x.categoria;document.getElementById("i-stock").value=x.stock;document.getElementById("i-stock-min").value=x.stock_minimo;document.getElementById("i-pcomp").value=x.precio_compra;document.getElementById("i-pvent").value=x.precio_venta;document.getElementById("i-proveedor").value=x.proveedor||"";document.getElementById("i-caducidad").value=x.fecha_caducidad||"";regMH("inventario",submitInventario);const frm=document.getElementById("form-inventario");frm.onsubmit=async function(ev){ev.preventDefault();try{await api("/inventario/"+id,{method:"PUT",body:JSON.stringify({nombre:document.getElementById("i-nombre").value,categoria:document.getElementById("i-categoria").value,stock:parseInt(document.getElementById("i-stock").value)||0,stock_minimo:parseInt(document.getElementById("i-stock-min").value)||5,precio_compra:parseFloat(document.getElementById("i-pcomp").value)||0,precio_venta:parseFloat(document.getElementById("i-pvent").value)||0,proveedor:document.getElementById("i-proveedor").value||null,fecha_caducidad:document.getElementById("i-caducidad").value||null})});showToast("Articulo actualizado");closeModal("inventario");loadInventario()}catch(err){showToast(err.message,false)}};openModal("inventario")}catch(e){showToast(e.message,false)}}
let serviciosCache=[];
let inventarioCache=[];
...
async function anularComprobante(id){if(!confirm('Anular este comprobante? El stock de productos se devolvera automaticamente.'))return;try{await api('/facturacion/comprobantes/'+id+'/anular',{method:'POST'});showToast('Comprobante anulado y stock devuelto');loadComprobantes()}catch(e){showToast(e.message,false)}}
